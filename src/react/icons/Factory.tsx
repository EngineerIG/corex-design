import * as React from 'react';

export interface FactoryProps extends React.SVGProps<SVGSVGElement> {
  size?: string | number;
}

const Factory = React.forwardRef<SVGSVGElement, FactoryProps>(
  ({ size = 24, ...props }, ref) => (
    <svg fill="currentColor" viewBox="0 0 24 24" width={size} height={size} ref={ref} {...props}><path d="M4 22c-.55 0-1.02-.196-1.413-.587A1.926 1.926 0 0 1 2 20v-8.7a1.993 1.993 0 0 1 1.225-1.85L7.6 7.6a1.01 1.01 0 0 1 .95.075c.3.183.45.458.45.825V9l3.625-1.45a.945.945 0 0 1 .938.1c.291.2.437.475.437.825V10h8v10c0 .55-.196 1.02-.587 1.413A1.926 1.926 0 0 1 20 22H4Zm0-2h16v-8h-8V9.95l-5 2V10l-3 1.325V20Zm7-2h2v-4h-2v4Zm-4 0h2v-4H7v4Zm8 0h2v-4h-2v4Zm7-8h-5l.9-7.125a.997.997 0 0 1 .325-.625c.183-.167.4-.25.65-.25h1.25c.25 0 .467.083.65.25a.997.997 0 0 1 .325.625L22 10Z" /></svg>
  )
);

Factory.displayName = 'Factory';

export default Factory;
